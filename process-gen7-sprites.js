...
// Add manual mappings for Pokemon not in raw-pokedex.json
nameToId['tapu-koko'] = 785;
nameToId['tapu-lele'] = 786;
nameToId['tapu-bulu'] = 787;
nameToId['tapu-fini'] = 788;
...
gen7Links.forEach(line => {
    ...
    if (form) {
        ...
        // Add generic slug mapping only if it looks like a form
        if (cleanName !== 'unknown') {
            newMapping[`${cleanName}-${formSlug}`] = url;
        }
        ...
        newMapping[cleanName] = url; // Add name based mapping for base forms too
