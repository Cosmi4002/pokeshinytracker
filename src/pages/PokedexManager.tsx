...
const previewFilterOverlay = (filter: CardFilterId) => {
    if (filter === "holo") {
        return (
            <div className="absolute inset-0 z-20 rounded-xl opacity-100 mix-blend-screen">
        ...
        return (
            <div className="absolute inset-0 rounded-xl mix-blend-screen">
                <div
                    ...
                    style={{
                        background:
                            "radial-gradient(circle at 18% 20%, rgba(129,140,248,0.28), transparent 34%), radial-gradient(circle at 80% 24%, rgba(236,72,153,0.18), transparent 30%), radial-gradient(circle at 50% 78%, rgba(56,189,248,0.18), transparent 36%)",
                    }}
                />
                {[
                    ["18%", "22%", "h-2.5 w-2.5 opacity-60"],
                    ["72%", "28%", "h-2 w-2 opacity-55"],
                    ["52%", "74%", "h-2.5 w-2.5 opacity-50"],
                ].map(([left, top, size], index) => (
                    <span
                        key={index}
                        className={`absolute ${size} -translate-x-1/2 -translate-y-1/2 rotate-45 rounded-[1px] bg-white shadow-[0_0_5px_rgba(255,255,255,0.70),0_0_10px_rgba(125,211,252,0.38)]`}
                        style={{ left, top }}
                    />
                ))}
            </div>
    ...
    const handleSaveCardSettings = async () => {
        ...
        try {
            ...
            toast({
                variant: "destructive",
                title: "Save error",
                description: error?.message || "Unable to save preferences card.",
            });
        } finally {
                                        ...
                                        {gamePresets.map((game) => (
                                            ...
                                            >
                                                ...
                                                <span className="min-w-0">
                                                    ...
                                                    <span className="block truncate text-xs text-muted-foreground">Apply to the selected game</span>
                                                </span>
                                            </button>
                                        ))}
                                    </div>
                            ...
                            <CardContent className="space-y-4 pt-5">
                                    ...
                                    <div className="relative z-10 flex min-h-[9.5rem] flex-col rounded-lg border border-white/10 bg-black/35 p-3">
                                        ...
                                        <div className="mt-auto grid grid-cols-3 gap-2 text-center">
                                            <div className="rounded-lg border border-white/10 bg-black/35 px-2 py-2">
                                                <p className="text-[10px] font-bold uppercase text-white/50">Method</p>
                                ...
                                <div className="grid gap-2">
                                    <Button type="button" onClick={handleSaveCardSettings} disabled={themesLoading || isSavingThemes}>
                                        ...
                                        {isSavingThemes ? "Saving..." : "Save card settings"}
                                    </Button>
