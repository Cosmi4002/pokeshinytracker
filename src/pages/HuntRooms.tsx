...
export default function HuntRooms() {
  ...
  // Realtime events are the fast path. Polling is a resilient fallback for
  // Supabase projects where Postgres Changes publication is not enabled yet.
  useEffect(() => {
  ...
  }, [isOnline, loadCurrentRoom, roomId, user]);

  const handleCreate = async () => {
    if (!selectedPokemonId || !selectedPokemonName || !roomName.trim()) return;
    setSubmitting(true);
    const spriteUrl = getPokemonSpriteUrl(selectedPokemonId, {
      shiny: true,
