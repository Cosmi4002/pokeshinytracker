...
export function getPokemonSpriteUrl(pokemonId: number, options: { shiny?: boolean, name?: string, female?: boolean, form?: string, animated?: boolean } = {}): string {
  ...
  // Ignore female flag for Pokémon that do not have gender differences (prevents white square for female where not available)
  const NO_GENDER_DIFF_IDS = [
  ...
  // If the Pokémon does not have gender differences, ignore the female flag
  if (NO_GENDER_DIFF_IDS.includes(pokemonId)) {
    female = false;
  }
